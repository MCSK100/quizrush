    ...
    <div className="mx-auto w-full min-w-0 max-w-md px-4 py-8 sm:px-5 sm:py-12">
      ...
      <div className="qr-surface mt-6 rounded-[24px] p-5 sm:p-6">
        ...
        <input id="code" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="Q7X9K" maxLength={6}
          className="font-num mt-2 w-full rounded-2xl border-[3px] border-dashed bg-white/90 px-4 py-4 text-center text-3xl font-bold tracking-[0.2em] text-ink outline-none placeholder:text-faint" style={{ borderColor: 'rgba(255,107,74,0.45)' }} />
        ...
        <div className="mt-2 flex flex-wrap gap-2">{AVATARS.slice(0, 8).map((a) => (
          <button key={a} onClick={() => setAv(a)} aria-pressed={av === a} className="btn-press grid h-11 w-11 place-items-center rounded-2xl bg-white text-xl shadow-sticker-sm" style={{ border: av === a ? '1.5px solid rgba(255,107,74,0.6)' : '1px solid rgba(120,100,180,0.08)' }}>{a}</button>
        ))}</div>
        {err && (
        ...
        <button onClick={join} className="qr-btn-primary mt-5 w-full justify-center rounded-2xl py-4 font-display text-base">JOIN GAME →</button>
      </div>
    </div>
  );
}
