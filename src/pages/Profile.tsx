...
{games===0?(<div className="mt-6 rounded-xl border border-[#2D1B4E]/10 bg-white/80 p-8 text-center"><p className="font-black">No matches yet.</p><p className="text-sm text-muted">Your quiz journey starts now.</p><Link to="/solo" className="btn-press mt-4 inline-block rounded-lg bg-neon px-6 py-3 font-black text-white">PLAY SOLO</Link></div>):
...
<div className="mt-3 rounded-xl border border-[#2D1B4E]/10 bg-white/80 p-4"><div className="text-xs font-black tracking-widest text-muted">FAVORITE TOPIC · {String(fav).toUpperCase()}</div>
